...
export const descentReducer = (hasChildren, getChildren) => {
	const reducer = reducerFn => init => node => {
		...
		// console.log('children:', getChildren(node))

		return getChildren(node).reduce(reducer(reducerFn), acc);
		// return reducer(reducerFn)(acc)(getChildren(node));
	}
	return reducer;
}
...
const intermediateObject = transformAll(someAggQuery)
*/
