<script>
	...
	function getEditor (targetDataType, targetTypeObject) {
		let simplifiedTypeObject = extractBaseType(targetTypeObject);
		if (targetDataType in typeEditors) {
			return typeEditors[targetDataType]
		} else if (simplifiedTypeObject && isStringLiteralUnion(simplifiedTypeObject)) {
			const unionEditor = {
				component: RadioList,
				props: {
			...
			return unionEditor;
		} else if (isArrayType(targetDataType)) {
			let subType;
			if (typeof simplifiedTypeObject === 'string' && simplifiedTypeObject.endsWith('[]')) {
				subType = simplifiedTypeObject.substr(0, simplifiedTypeObject.length - 2);
			} else if (simplifiedTypeObject && '__array' in simplifiedTypeObject) {
				subType = simplifiedTypeObject.__array;
			} else {
			...
			return arrayEditor;
		} else if (isJsonType(targetDataType) || isRecordType(simplifiedTypeObject)) {
			return {
				component: SimpleField,
				props: {
					type: 'json',
		...
		return  {
			...
			props: {
				type: 'json',
				dataType: targetDataType,
				typeObject: simplifiedTypeObject
			}
		};
	}
