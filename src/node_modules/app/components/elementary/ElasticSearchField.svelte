<script>
	...
	const makeTrimEndChars = count => string =>
		string.substr(0, string.length - count);
	const isOf = obj => key => key in obj;
	const getFrom = obj => key => obj[key];
	const hasUnits = _.hasKey('__units');
	const makeConfigEditor = component => props => ({component, props});
	const configUnionEditor = type => makeConfigEditor(RadioList)({
		typeObject: type
	});
	const configArrayEditor = type => makeConfigEditor(ArrayEditor)({
	...
	const configQuantityEditor = ({__units}) => makeConfigEditor(QuantityEditor)({
		unitList: __units
	});
	const configJsonEditor = type => makeConfigEditor(SimpleField)({
		type: 'json',
		typeObject: type
	});
	const configObjectEditor = type => makeConfigEditor(ObjectEditor)({
		typeObject: type
	});
	...
	const getEditor = _.pipe([
		...
		_.adapter([
			_.casus(
				_.allOf([isString, isOf(typeEditors)]),
				getFrom(typeEditors)
			),
			...
			_.casus(hasUnits, configQuantityEditor),
