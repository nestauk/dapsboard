<script>
	...
	function isStringUnion (type) {
		const paramNames = Object.keys(type);
		return paramNames.every(name => {
			const subType = type[name];
			return typeof subType === 'string' && subType.startsWith('"');
		});
	}
	...
	function getEditor (targetDataType, targetTypeObject) {
		if (targetDataType in typeEditors) {
			...
			let subType;
			if (targetTypeObject && '__array' in targetTypeObject) {
