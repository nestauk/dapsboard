<script>
	...
	function getEditor (targetDataType, targetTypeObject) {
		console.log("getting Editor", targetDataType, targetTypeObject)
		if (targetDataType in typeEditors) {
			return typeEditors[targetDataType]
		} else if (targetTypeObject && isStringUnion(targetTypeObject)) {
			const unionEditor = {
				component: RadioList,
				props: {
			...
			let subType;
			if (typeof targetTypeObject === 'string' && targetTypeObject.endsWith('[]')) {
				subType = targetTypeObject.substr(0, targetTypeObject.length - 2);
			} else if (targetTypeObject && '__array' in targetTypeObject) {
				subType = targetTypeObject.__array;
			} else {
