<script>
	...
	import { is_xor, is_intWithUnit } from 'app/elasticsearch/types/genericTypes';
...
{#each aggParamsInfo as paramInfo (`${id}-${paramInfo.name}`)}
	{#if !isParamSelector
		|| ['__selection', value?.__selection].includes(paramInfo.name)
	}
		<TypedField
			labelText={isParamSelector || isIntWithUnits ? '' : paramInfo.name}
