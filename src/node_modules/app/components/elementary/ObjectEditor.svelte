<script>
	...
	import { computeParamInfo } from 'app/elasticsearch';

	import ESField from './ElasticSearchField.svelte';
	// import { isKeyValue } from '@svizzle/utils';  // doesn't seem to work
	...
	// const isNameType = isKeyValue('name', '__type') // doesn't seem to work
	const isNameType = param => param.name === '__type';

	function computeTypings () {
		console.log(typeObject);
		let type = typeObject;
		if (Array.isArray(type)) {
			type = type.filter(t => t !== 'undefined')
			if (type.length > 1) {
