...
function generateHTML (input) {
	...
	switch (nodeType) {
		...
		case 'object': {
			...
			let propsHtml = keys.map((k, i) => `<li><span class='property-name'>"${k}"</span> <span class='boilerplate'>:</span> <span class='property-value'>${generateHTML(input[k])}</span>${i < keys.length - 1 ? "<span class='boilerplate'>,</span>" : ""}</li>`);
