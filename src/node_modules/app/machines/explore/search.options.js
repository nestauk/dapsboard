...
//	cache[]
const setStatsCacheKey = (ctx, event) =>
	console.log('hey!')
const isInStatsCache = ({statsCacheKey}) => false; // get(statsCachy) in cache;
const updateStatsCache = ({statsCacheKey}, {data}) => {
	statsCache[statsCacheKey] = data
};

function computeSelection (fieldStats, userSelection) {
...
async function sendSuggestionsRequest (ctx, event) {
	...
	// 1. request
	const info = get(ctx.fieldStats).find(fieldInfo => fieldInfo.name === fieldName)
	if (info.suggestions) {
		return;
	}
...
function onFieldSelected (event) {
	...
	selectedFieldName = computeSelection(fieldStats);
}
function selectNext () {
	...
	if (index < fieldStats.length) {
		userSelection = fieldStats[index].name;
		selectedFieldName = computeSelection(fieldStats);
	}
}
