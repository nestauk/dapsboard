...
const updateQuery = ({isFieldsMenuActive}) => {
	isFieldsMenuActive.set(false);
	return {isFieldsMenuActive};
}
...
const updateSuggestions = ({
	...
	// TODO make functional
	const buckets = (data.aggregations[get(selectedFieldName)]?.buckets || [])
	.filter(sugg => sugg.key !== get(searchQuery))
	.sort((a, b) => b.doc_count - a.doc_count);
	const $suggestions = buckets.map(sugg => `${sugg.key} (${sugg.doc_count})`);
...
export const searchOptions = {
	actions: {
		...
		selectPrevField: assign(selectPrevField),
