...
function processRequestValue (value, type) {
	...
	if (is_intWithUnit(type)) {
		return `${value.__value}${value.__unit}`;
	}
	if (is_object(type) || is_extent(type) || is_xor(type)) {
		...
		typings.forEach(typing => {
			...
			const result = processRequestValue(obj, currentType);
			if (
				isNotNil(result)
...
function computeRequestInternal (
	...
	const allParams = { // FIXME need deep copy
		...extendedParams,
		...basicParams,
	};
	const outgoing = processRequestValue(allParams, getAggType(dataset, selection));
