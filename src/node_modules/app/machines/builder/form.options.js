...
import {isNotNull, isNotNil, isObjEmpty} from '@svizzle/utils';
...
function computeTypings (ctx) {
	...
	ctx.aggParamsInfo.set(params);

	return ctx;
}

/*
const getStoreValue = store => get(store);
const getContextStoreValueFor = storeName => _.pipe([
	_.getKey(storeName),
	getStoreValue,
]);
const getDataset = datasetId => DATASETS.find(ds => datasetId === ds.id);
const getSelection = getContextStoreValueFor('selection');
const getSelectionFor = item => _.pipe([
	getSelection,
	_.getKey(item)
]);
const getSelectedField = getSelectionFor('field');
const getSeletedAggregation = getSelectionFor('aggregation');

const getSelectedDatasetSchema = _.pipe([
	getContextStoreValueFor('dataset'),
	getDataset,
	getSchema
]);
const getDatasetField = ctx =>
	getSelectedDatasetSchema(ctx)[getSelectedField(ctx)];
const getSelectedFieldType = _.pipe([
	getDatasetField,
	getESType
]);

const computeTypings = ctx => {
	getParamsByAgg(getSeletedAggregation(ctx), getSelectedFieldType(ctx));
	return ctx;
};
*/

const extractDefaultValue = _.adapter([
	_.casus(_.hasKey('__default'), _.getKey('__default')),
	_.casus(
		_.not(is_native),
		_.pipe([
			getTypings,
...
const extractDefaultValues = _.pipe([
	...
	// tapValue('processed'),
	_.casus(_.pipe([_.getAt(1), isNotNil]), _.fromPairs),
	// tapValue('objectified'),
]);
function computeDefaultValuesInternal (aggParamsInfo) {
	if (aggParamsInfo.length === 0) {
		return undefined;
	}
	return extractDefaultValues(aggParamsInfo);
}
...
function processRequestValue (params, name, value, type) {
	...
	if (!value) {
		return
	} else if (is_object(type) || is_xor(type) || is_extent(type)) {
		if (isObjEmpty(value)
			&& !(
				is_optional(type)
				&& '__default' in type
				&& isObjEmpty(type.__default)
			)
		) {
			delete params[name];
		}
		const info = getTypings(type);
		info.forEach(prop => {
			processRequestValue(
				value,
				prop.name,
	...
	} else if (is_intWithUnit(type)) {
		params[name] = `${value.value}${value.unit}`;
	}
	if (is_xor(type)) {
		const fieldName = value.selection;
		params[fieldName] = params[name][fieldName];
		delete params[name];
	}
}
...
function computeRequestInternal (
	...
	const computedParams = JSON.parse(JSON.stringify({ // FIXME need deep copy
		...extendedParams,
		...basicParams
	}));
	const agg = {
	...
	isRequiredSet = isRequiredSet && aggParamsInfo.every(c => {
		const params = computedParams[c.name];
		return c.name === 'field'
			|| !c.required
			|| is_xor(c.type)
			&& isNotNil(params?.[params?.selection])
			|| isNotNil(params)
	});
	console.log('basicParams', basicParams)
	console.log('extendedParams', extendedParams)
	console.log('computedParams', computedParams)
	aggParamsInfo.forEach(param => {
		processRequestValue(
			computedParams,
			param.name,
			computedParams[param.name],
			param.type
		);
	});
	console.log('computedParams', computedParams)
