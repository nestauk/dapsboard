...
function isFormActive (ctx) {
	const selectedForm = get(ctx.selectedForm);
	return selectedForm && ctx.id === selectedForm.name;
}
...
function computeRequestInternal (
	...
	const fieldInfo
		= getSchema(DATASETS[selection.dataset])[selection.field];
	const agg = buildAggregation(
...
function computeRequest (ctx, event) {
	...
	if (isFormActive(ctx)) {
		...
		const endpoint = getEndpointURL(DATASETS[activeSelection.dataset]);
