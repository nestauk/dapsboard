...
function doQuery (ctx) {
	...
	const endpoint = getEndpointURL(DATASETS[get(ctx.activeSelection).dataset]);
	const url = `${endpoint}/_search`;
	ctx.cacheKey = `${url}/${JSON.stringify(ctx.activeQuery)}`;
	return request('POST', url, {data: get(ctx.activeQuery)});
}
