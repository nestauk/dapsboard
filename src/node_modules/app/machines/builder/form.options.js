...
const makeChildrenReducer = value => (acc, typing) => {
	let key = typing.name;
	let obj = value[key];
	if (obj === undefined) {
		return acc;
...
function processRequestValue (value, type) {
	...
	if (is_object(type) || is_extent(type) || is_xor(type)) {
		const typings = getParamsInfo(type);
		const cookedParams = _.reduce(typings, makeChildrenReducer(value), {});
		return cookedParams;
	}
	return undefined;
}
...
function computeRequestInternal (
	...
	isRequiredSet = isRequiredSet && aggParamsInfo.every(info => {
		const params = rawParams[info.name];
