...
/*
function setURL (ctx) {
	const name = get(ctx.dataset)
	const query = rison.encode(getQuery(ctx));
	const url = `${window.location.pathname}?dataset=${name}&q=${query}`;
...
export const formOptions = {
	actions: {
		 ...
		 * Clear the route.
		 */
		clearURL: assign(clearURL),
		/**
		 * Update the browser history after successful request.
		 */
		pushHistory: assign(pushHistory),
		/**
		 * Rename the form.
		 */
