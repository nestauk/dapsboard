...
const evaluate = _.when(isFunction, _.applyTo(['dummyType']));

const mergeDocs = (info, docs) => {
	info.forEach(param => {
		param.documentation = docs[param.name];
	});
	return info;
};
...
export function getParamsByAgg (agg, type) {
	...
	const computedParams = evaluate(params);
	const info = getParamInfo(computedParams);
	const docs = docsByParamByAgg[agg];
