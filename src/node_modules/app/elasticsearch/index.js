...
export const isSimpleType = type => typeof type === 'string';
export const isTypeUnion = type => Array.isArray(type);
export const isCompoundType = type => typeof type === 'object';
// export const isArrayType = type => '__array' in type; // TODO: new test
export const isArrayType = makeEndsWith('[]');
export const isJsonType = type => type === 'json';
export const isRecordType = type => '__record' in type;


function getDisplayType (fullType, indentation = 0) {
	if (isSimpleType(fullType)) {
		return fullType;
	}
	if (isTypeUnion(fullType)) {
		return fullType
		.filter(type => type !== 'undefined')
		.map(type => getDisplayType(type, indentation + 1))
		.join(' | ');
	}
	if (isCompoundType(fullType)) {
	...
	throw new Error('Failed to parse type data, bad type definition.');
}

export function getParamInfo (params) {
	const paramNames = Object.keys(params);
	return paramNames.map(name => {
		const paramType = params[name];
		const required = !(Array.isArray(paramType) && paramType.includes('undefined'));
		let presentationType = getDisplayType(paramType);
		return {
			name,
			type: paramType,
			required,
