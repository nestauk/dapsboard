...
export export type MatrixAggregations<Dataset, Fields extends keyof Dataset> = {
    matrix_stats: {
        fields: Fields[]
        missing?: {
...
// user code
interface SomeDataset {
	firstField: string;
	secondField: number;
};

type SomeDatasetField = keyof SomeDataset;

type Test = SomeDataset[SomeDatasetField]
type Test1 = MetricField<SomeDataset>
type SomeAggs = Explode<Aggregations<SomeDataset, SomeDatasetField>>;

const aggFieldConf: MetricAggregationConfig<SomeDataset, SomeDatasetField> = {
	field: 'secondField',
    missing: 4
};
console.log(aggFieldConf);


const aggFieldConf1: MetricAggregationConfig<SomeDataset, SomeDatasetField> = {
	field: 'firstField',
    missing: 'dfsf',
};
console.log(aggFieldConf1);

const aggFieldConf2: MetricAggregationConfig<SomeDataset, SomeDatasetField> = {
    script: 'fsdf'
};
console.log(aggFieldConf2);

const aggsConfig: Aggs<SomeDataset, SomeDatasetField> = {
	aggName: {
		avg: {
			field: 'secondField',
			missing: 4
