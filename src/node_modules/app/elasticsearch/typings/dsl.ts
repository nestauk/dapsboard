...
export type DateInterval = '1ms' | '1s' | '1m' | '1h' | '1d' | '1w' | '1M' | '1q' | '1y';

export type CalendarInterval = '1m' | '1h' | '1d' | '1w' | '1M' | '1q' | '1y';

export type FixedInterval = '1ms' | '1s' | '1m' | '1h' | '1d';
...
export type MetricAggregations<Dataset, Fields extends keyof Dataset> = {
     ...
     */
    median_absolute_deviation: MetricAggregationConfig<Dataset, Fields> & {
        /**
         * The tradeoff between resource usage and accuracy of a TDigest’s
         * quantile approximation, and therefore the accuracy of this aggregation’s
         * approximation of median absolute deviation, is controlled by the
     ...
     * follows a Student’s t-distribution under the null hypothesis on numeric 
     * values.
     */
    t_test: {
        /**
...
export type BucketAggregations<Dataset, Fields extends keyof Dataset> = {
    ...
    /**
     * A multi-bucket aggregation similar to the Date histogram aggregation except
     * instead of providing an interval to use as the width of each bucket, a target
     * number of buckets is provided indicating the number of buckets needed and the
     * interval of the buckets is automatically chosen to best achieve that target.
     * The number of buckets returned will always be less than or equal to this
     * target number.
     */
    auto_date_histogram: MetricAggregationConfig<Dataset, Fields> & {
        /**
        ...
        buckets?: integer;
        /**
         * Date format pattern specification.
         */
        format?: string;
        /**
         * The time_zone parameter can be used to indicate that bucketing should
    ...
    date_histogram: MetricAggregationConfig<Dataset, Fields> & {
        ...
        // with each other. The value of the `__type` property indicates which is the 
        // actual property to include in the final request JSON.
        // This is useful when these fields are required, but mutually exclusive.
        // All inner properties inherit the value from the parent private property.

        /**
         * Select among `interval` for ES < 7 datasets and `calendar_interval` and
         * `fixed_interval` for ES >= 7.
         */
        __interval: {
            ...
            /**
             * Fixed intervals are a fixed number of SI units and never deviate,
             * regardless of where they fall on the calendar. One second is always
             * composed of 1000ms. This allows fixed intervals to be specified in
    ...
    /**
     *  multi-bucket value source based aggregation which finds "rare"
     * terms — terms that are at the long-tail of the distribution and
     * are not frequent.
     */
    rare_terms: MetricAggregationConfig<Dataset, Fields> & {
        /**
         * The maximum number of documents a term should appear in.
         */
        max_doc_count?: integer;
        /**
         * The precision of the internal CuckooFilters. Smaller precision 
         * leads to better approximation, but higher memory usage. 
         * Cannot be smaller than 0.00001. Default 0.01
         */
        precision?: float;
        /**
